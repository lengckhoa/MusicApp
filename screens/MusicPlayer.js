...
const setupPlayer = async (songs) => {
  try {
    ...
    //     artwork: songData.imgURL,
    //     url: songData.songURL,
    //   });
    // });

    // Thiết lập player và thêm các bài hát
...
const togglePlayBack = async playBackState => {
  ...
  if (currentTrack !== null) {
    if (playBackState === State.Paused || playBackState === State.Ready || playBackState === State.Connecting || playBackState === State.Buffering) {
  ...
  useEffect(() => {
    const getSongs = async () => {
      try {
        ...
        const songsData = [];
        querySnapshot.forEach((doc) => {
          // Add the entire songs array to songsData
          songsData.push(...doc.data().songs);
        });
